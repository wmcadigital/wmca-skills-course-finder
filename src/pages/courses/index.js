...
const searchCourses = (courses, searchTerm) => {
  ...
  return courses.filter((course) => searchTermRegex.test(course.CourseName));
};
...
const filterCourses = (courses, studyModes, filterType) => {
  ...
  return courses.filter((course) =>
    updatedStudyModes.includes(course[filterType])
  );
};
...
const Page = () => {
  ...
  useEffect(() => {
    ...
    window.history.replaceState(
      {},
      "",
      `#/course-finder?${newSearchParams.toString()}`
    );
  }, [currentPage, filter]);
  ...
  useEffect(() => {
    ...
    accordionData.map((category) => {
      const arrayToCheck =
        category.type === "courseHours"
          ? filter.courseHours
          : category.type === "courseType"
          ? filter.courseType
          : category.type === "courseStudyTime"
          ? filter.courseStudyTime
          : [];
    ...
    if (filter.courseStudyTime.length) {
      ...
      coursesFiltered = filterCourses(
        coursesFiltered,
        filter.courseStudyTime,
        "AttendancePatternType"
      );
    }

    if (filter.startDate) {
      setCurrentPage(1);
      coursesFiltered = filterCoursesByStartDate(
        coursesFiltered,
        filter.startDate
      );
    }
  ...
  const courseDetailsLink = (e, course) => {
    ...
    e.preventDefault();
    navigate(
      `/course-finder/details?courseId=${course.CourseID}&locationName=${course.LocationName}&startDate=${course.StartDate}&durationValue=${course.DurationValue}`
    );
  };

  const handleCheckboxChange = (accordionIndex, checkboxIndex) => {
    const updatedAccordionData = [...accordionData];
    updatedAccordionData[accordionIndex].checkbox[checkboxIndex].checked =
      !updatedAccordionData[accordionIndex].checkbox[checkboxIndex].checked;

    // The array you want to push into courseType
    const newArray =
      updatedAccordionData[accordionIndex].checkbox[checkboxIndex];
  ...
  const coursesCountAmount = () => {
    ...
    if (coursesCount > 1) {
      ...
      return (
        <p className="course-count">
          <strong>{coursesCount}</strong> results found
        </p>
      );
    }
  };
  ...
  const generatePageIndexPagination = () => {
      ...
      Li.push(
        <li key={1} className="wmcads-pagination__item">
          {1 === currentPage ? (
            ...
            <a href="#0" onClick={(e) => handleJumpToPage(e, 1)} className="wmcads-link">
              1
            </a>
          )}
        </li>
      ...
      Li.push(
        <li key={i} className={"wmcads-pagination__item"}>
          {i === currentPage ? (
            ...
            <a href="#0" onClick={(e) => handleJumpToPage(e, i)} className="wmcads-link">
              {i}
            </a>
          )}
        </li>
    ...
    if (endPage < totalPagesForcourses) {
      ...
      Li.push(
        <li
          key={totalPagesForcourses}
          className={
            totalPagesForcourses === currentPage
              ? "wmcads-pagination__item"
              : "wmcads-pagination__item"
          }
        >
          {totalPagesForcourses === currentPage ? (
            ...
            <a
              href="#0"
              onClick={(e) => handleJumpToPage(e, totalPagesForcourses)}
  ...
  const CoursesFound = (currentCourseItems) => {
    ...
    return (
      <div>
        {loading ? (
          <div className="load-wrapper-main">{contentLoad}</div>
          ...
          currentCourseItems.map((course, index) => (
            <div key={index} className="wmcads-search-result">
              <h2 className="wmcads-m-b-none">
                <a
                  href={newTabWindowLink(course)}
                  onClick={(e) => courseDetailsLink(e, course)}
              ...
              <p className="mtb-10">
                <strong>{course.LocationName}</strong>
              </p>
              ...
              <table className="courses wmcads-table wmcads-m-b-xl wmcads-table--without-header">
                <tbody>
                  <tr>
                    <td className="" data-header="Header 2">
                      <strong>Start date: </strong>
                      {isFlexible(course)}
                    </td>
                    <td className="" data-header="Header 2">
                      <strong>Course type: </strong>
                      {course.DeliverModeType}
                    </td>
                  </tr>
                  <tr>
                    <td className="" data-header="Header 2">
                      <strong>Cost: </strong>
                      {course?.CostDescription}
                    </td>
                    <td className="" data-header="Header 2">
                      <strong>Duration: </strong>
                      {corsesDuration(course)}
                    </td>
                  </tr>
                </tbody>
              </table>
          ...
          <div
            className="wmcads-msg-summary wmcads-msg-summary--warning"
  ...
  return (
    <div className="template-search">
      <div className="wmcads-m-b-lg">
        <h1 id="wmcads-main-content">
          Find a funded course for adults in the West Midlands
        </h1>
      ...
      <div className="wmcads-grid">
        <div className="main wmcads-col-1 wmcads-col-md-2-3 wmcads-m-b-xl wmcads-p-r-lg wmcads-p-r-sm-none">
          ...
          <div
            className="wmcads-hide-desktop mobile-sticky-btn"
            ...
            >
              <div className="wmcads-search-filter__header">
                ...
                {filterIsModified && (
                  <a
                    href="#0"
                    className="wmcads-hide-desktop wmcads-link wmcads-hide-desktop hide-desktop"
              ...
              {filterIsModified && (
                <div
                  className="wmcads-container wmcads-hide-desktop"
                  onClick={toggleMobileFilters}
                >
                  <button
                    className="wmcads-btn wmcads-btn--primary wmcads-btn--block"
